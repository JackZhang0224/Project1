...
Line2 <- RxXdfData("Pi_Schedule2.xdf")
...
train <- rxImport(inData = as.data.frame(train_test[1]), outFile='Line2_train.xdf', stringsAsFactors = T, overwrite=T)
test <- rxImport(inData = as.data.frame(train_test[2]), outFile='Line2_test.xdf', stringsAsFactors = T, overwrite=T)
remove(Line2_df)
remove(train_test)

rxDataStep(train, outFile = "Line2_train1.xdf", rowsPerRead = 1000000)
file.remove("Line2_train.xdf")
file.rename("Line2_train1.xdf", "Line2_train.xdf")
train <- RxXdfData("Line2_train.xdf")
...
lm_scored <- rxPredict(lm_model, test, writeModelVars = T, outData = "Line2_lm_scored.xdf", overwrite = T)
...
glm_scored <- rxPredict(glm_model, test, writeModelVars = T, outData = "Line2_glm_scored.xdf", overwrite = T)
...
dt_scored <- rxPredict(dt_model, test, writeModelVars = T, outData = "Line2_dt_scored.xdf", overwrite = T)
...
s <- Sys.time()
df_model <- rxDForest(formula, nTree = 16, importance = T, data = train, reportProgress = 0,
